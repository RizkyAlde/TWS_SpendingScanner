...
// Query untuk mengambil pengeluaran setiap hari pada bulan ini
$query_this_month_daily = "SELECT DAY(date) AS day, SUM(product_price) AS total_this_month FROM report WHERE MONTH(date) = MONTH(CURDATE()) AND YEAR(date) = YEAR(CURDATE()) GROUP BY DAY(date)";
$result_this_month_daily = mysqli_query($koneksi, $query_this_month_daily);

// Query untuk mengambil pengeluaran setiap hari pada bulan lalu
$query_last_month_daily = "SELECT DAY(date) AS day, SUM(product_price) AS total_last_month FROM report WHERE MONTH(date) = MONTH(CURDATE() - INTERVAL 1 MONTH) AND YEAR(date) = YEAR(CURDATE() - INTERVAL 1 MONTH) GROUP BY DAY(date)";
$result_last_month_daily = mysqli_query($koneksi, $query_last_month_daily);

// Menginisialisasi array untuk menyimpan pengeluaran setiap hari pada bulan ini dan bulan lalu
$spending_this_month = array_fill(1, 31, 0);
$spending_last_month = array_fill(1, 31, 0);

// Mengisi array dengan data pengeluaran setiap hari pada bulan ini
while ($row = mysqli_fetch_assoc($result_this_month_daily)) {
    $day = $row['day'];
    $spending_this_month[$day] = $row['total_this_month'];
}

// Mengisi array dengan data pengeluaran setiap hari pada bulan lalu
while ($row = mysqli_fetch_assoc($result_last_month_daily)) {
    $day = $row['day'];
    $spending_last_month[$day] = $row['total_last_month'];
}
